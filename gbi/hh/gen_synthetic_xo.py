...
def generate_synthetic_xo():
    ...
    obs_stats_ls_prior = deepcopy(obs_stats_ls)
    all_obs_prior = deepcopy(obs_stats_ls)
    ...
    with open(
        "../../results/hh_allen/synthetic_xo/prior_pred_traces.pkl", "wb"
    ) as handle:
        pickle.dump(all_obs_prior[:10], handle)

    with open("../../results/hh_allen/synthetic_xo/prior_pred_xo.pkl", "wb") as handle:
        pickle.dump(obs_stats_ls_prior[:10], handle)
